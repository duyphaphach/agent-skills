...
function log(message) {
  console.log(`[utils-mcp-inject-config] ${message}`);
}

function fail(message) {
  console.error(`[utils-mcp-inject-config] ${message}`);
  process.exit(1);
}
...
function createPosixServerConfig(workspaceFolder) {
  ...
  const utilsScriptPath = path.join(
    workspaceFolder,
    "bin",
    "utils-mcp.js"
  );
  ...
  return {
    ...
    args: [
      ...
      `set -a && . ${formatSourcePath(
        envFilePath
      )} && set +a && node ${formatNodePath(utilsScriptPath)}`,
    ],
  };
}
...
function main() {
  ...
  const serverName = getArgValue("--server-name") ?? "utils-mcp-test";
