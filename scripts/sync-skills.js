...
/**
 * @module sync-skills
 * @description Discovers skills in the packages/skills directory and syncs their metadata
 * across all generated files: manifest.json, platform plugin configs
 * (.claude-plugin/plugin.json, .cursor-plugin/plugin.json),
 * .claude-plugin/marketplace.json, packages/skills/index.json, and README.md.
 *
...
function generateReadmeSkillsList(skills) {
  ...
  for (const skill of skills) {
    ...
    lines.push(
      `| [${skill.name}](./packages/skills/${skill.dirName}) | ${escapedDesc} |`
    );
  }
...
async function updateManifest(manifest, skills) {
  manifest.skills = skills.map((skill) => ({
    ...
    source: `./packages/skills/${skill.dirName}`,
...
// Generates packages/skills/index.json following the agent-skills-discovery RFC,
...
async function main() {
  ...
  if (indexUpdated) {
    console.log("Updated packages/skills/index.json");
  }
